...
namespace DoceCantinho.Desktop.Helpers
{
    ...
    public sealed class HttpClientHelper
    {
        ...
        // Substitua os métodos abaixo no HttpClientHelper por estas versões de diagnóstico:
        ...
        /// <summary>
        /// Desserializa o corpo da resposta de forma segura.
        /// Respostas de sucesso sem corpo (ex.: 204 No Content) são comuns em
        /// operações PUT/DELETE — nesse caso retornamos default(T) em vez de
        /// lançar exceção, para não transformar um sucesso em falso erro.
        /// </summary>
        private T? DeserializeSafely<T>(string responseBody)
        {
        ...
        public async Task<(bool Success, T? Data, string ErrorMessage)> PostAsync<T>(string endpoint, object body)
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    var data = DeserializeSafely<T>(responseBody);
                    return (true, data, string.Empty);
                }

                var error = $"HTTP {(int)response.StatusCode} {response.ReasonPhrase}: {TryExtractErrorMessage(responseBody)}";
                System.Diagnostics.Debug.WriteLine($"[POST] {endpoint} -> {error}");
        ...
        public async Task<(bool Success, T? Data, string ErrorMessage)> PutAsync<T>(string endpoint, object body)
        {
            try
            {
                var json = JsonSerializer.Serialize(body);
                ...
                var error = $"HTTP {(int)response.StatusCode} {response.ReasonPhrase}: {TryExtractErrorMessage(responseBody)}";
                System.Diagnostics.Debug.WriteLine($"[PUT] {endpoint} -> {error}");
                return (false, default, error);
            }
            catch (Exception ex)
            {
