    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginDto dto, string? returnUrl = null)
        {
            ...
            if (result.Succeeded)
            {
                ...
                // Se houver returnUrl, só redireciona para áreas administrativas
                // quando o usuário realmente for Admin. Caso contrário, ignora o returnUrl
                if (!string.IsNullOrEmpty(returnUrl) && Url.IsLocalUrl(returnUrl))
                {
                    var isAdminArea = returnUrl.StartsWith("/Admin", StringComparison.OrdinalIgnoreCase);
                    if (isAdminArea)
                    {
                        if (user != null)
                        {
                            ...
                            // usuário não é admin: não permite acesso ao admin mesmo quando havia returnUrl
                ...
                //Verifica roles do usuário para redirecionar Admins ao painel
                if (user != null)
                {
                    var roles = await _userManager.GetRolesAsync(user);
                    if (roles.Contains("Admin"))
        ...
        public async Task<IActionResult> Register(RegisterDto dto)
        {
            ...
            var user = new ApplicationUser
            {
                UserName = dto.Email,
                Email = dto.Email,
                // Campos obrigatórios com valores padrão — podem ser editados depois no perfil
                Cpf = "000.000.000-00",
            ...
            if (result.Succeeded)
            {
                ...
                // Garante que o usuário receba a role padrão "Usuário"
